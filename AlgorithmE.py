...
def algorithm_E_sup(set_A, set_B, portals_of_A, adj, distances_from_separator, rangeTree, initial_graph, tw, tree_decomp_graph, test, A, B):
    ...
    # Recurse
    e_x_X = {}
    e_x_X = algorithm_E(H, tw, portals_of_A2, set_A2, set_B2, tree_decomp_graph, A2, B2)
...
def algorithm_E(initial_graph, tw, portals_of_A, set_A, set_B, tree_decomp_graph, A, B):
    """
...
def main():
    ...
    writer.writerow(['Treewidth', 'Dimension', 'Log(n)', 'First term', 'Second Term', 'n', 'Diameter', 'Time'])
    ...
    for i in range(1, len(sys.argv)):
        ...
        #G = nx.connected_caveman_graph(50, 6)
        start = time.time()
        print("Start timing")
        print("------------")
        ...
        eccentricities = algorithm_E(initial_graph, p[0], portals_of_A, set_A, set_B, tree_decomp_graph, A, B)
